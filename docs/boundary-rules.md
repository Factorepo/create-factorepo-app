...
7. Keep every constant values in [packages/constants.md](packages/constants.md).
...
4. Never catch an exception, except when you try the operation again, or when
   the external system reports that an item is absent.
5. Never write a function that does more than one action.
6. Never return a null value or an empty value when an operation fails. Throw an
   exception instead.
