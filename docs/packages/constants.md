...
2. Give a group of values that share one shape its own folder, built from its
   member files in `index.ts`.
3. Export every public name from the index file, with the value and the type in
   two statements.
...
4. Never repeat the number of a constant inside a string. Build the string with
   a template literal.

## 4. Values

1. Name a constant in upper snake case, and write `as const` on every literal
   array and object.
2. Write a byte size as a multiplication of readable factors.
3. Add a function here only when it parses or formats a constant of this
   package, and keep it pure.
...
5. Write a plain text body with each HTML body, and keep the two to the same
   wording.
