...
3. Report the conflict when a document and the code disagree, and never follow
   the code in silence.
...
7. Give every markdown file under `apps` or `packages` these sections, in this
   order: **Responsibility**, **Layout**, **May use**, **Must not**, then its
   own sections. Omit a section when a parent rule file already owns it.
